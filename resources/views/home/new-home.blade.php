...
@section('content')
                        ...
                        @if ($s->id % 2)
                            <li data-transition="fade" class="slide_show"> <!-- Slide_show -->
                                ...
                                <div class="main_heading tp-caption tp-resizeme title_container"
                                    ...
                                    data-voffset="['-153','-163','-183','-173']" data-whitespace="nowrap"
                                ...
                                <div class="tp-caption tp-resizeme" data-x="['left','left','left','left']"
                                    ...
                                    data-voffset="['-51','-53','-63','-75']" data-transform_idle="o:1;"
                                    data-whitespace="nowrap" data-transform_in="x:50px;opacity:0;s:1000;e:Power3.easeOut;"
                                    data-transform_out="s:1000;e:Power3.easeInOut;s:1000;e:Power3.easeInOut;"
                                    data-start="1500">
                                    <h1>{{ substr($s->description, 0, 10) }}<br>{{ substr($s->description, 10, 20) }}<br>{{ substr($s->description, 20, 30) }}
                                    </h1>
                                </div>
                            </li> <!-- /Slide_show -->
                        @else
                            <li data-transition="slideright" class="slide_show slide_3"> <!-- Slide_3 -->
                                ...
                                <div class="main_heading tp-caption tp-resizeme title_container"
                                    ...
                                    data-y="['middle','middle','middle','middle']"
                                    data-voffset="['-150','-150','-160','-150']" data-transform_idle="o:1;"
                                    data-transform_in="x:-50px;opacity:0;s:2000;e:Power3.easeOut;"
                                    data-transform_out="s:1000;e:Power3.easeInOut;s:1000;e:Power3.easeInOut;"
                                    data-whitespace="nowrap" data-start="1300" data-splitin="none" data-splitout="none"
                                    data-responsive_offset="on">
                                    <h6>{{ $s->title }}</h6>
                                    <span class="decor_default" style="margin-top:19px;"></span>
                                </div>
                                <div class="tp-caption tp-resizeme" data-x="['right','right','right','right']"
                                    data-hoffset="['0','85','85','44']" data-y="['middle','middle','middle','middle']"
                                    data-voffset="['-50','-50','-60','-50']" data-transform_idle="o:1;"
                                    data-transform_in="x:50px;opacity:0;s:1000;e:Power3.easeOut;"
                                    data-transform_out="s:1000;e:Power3.easeInOut;s:1000;e:Power3.easeInOut;"
                                    data-whitespace="nowrap" data-start="1400" data-splitin="none" data-splitout="none"
                                    data-responsive_offset="on">
                    ...
                    @foreach ($promo as $p)
                        <div class="finance_fact_item row">
                            <div class="icon_holder col-lg-2 col-md-2 col-sm-3 col-xs-2">
                                <span style="font-size: 55px;color: #{{ $general->color }}"
                                    class="icon">{!! $p->icon !!}</span>
                            </div>
                            <div class="finance_fact_name col-lg-7 col-md-7 col-sm-7 col-xs-6">
                                <h6>{{ $p->title }}</h6>
                                <span>{{ $p->s_text }}</span>
                            </div>
                            <span class="col-lg-3 col-md-3 col-sm-2 col-xs-4 counter timer" data-from="0"
                                data-to="{{ $p->number }}" data-speed="5000" data-refresh-interval="50"></span>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="invest-step">
                        ...
                        <p class="text">Free registration. To open an account, it's sufficient to enter your e-mail. and
                            phone
                        </p>
                    </div>
                </div>
    ...
    <section class="invest-types">
        <div class="container text-center">
            <h2 class="page__title page__title--3 text-center page__title--plans"><span class="c-blue">Investment
                    Plan</span></h2>
            <div class="row">
                @foreach ($plan as $p)
                    <div class="col-md-4 ">
                        <div class="invest-type">
                            ...
                            <div class="invest-type__profit plan__value--{{ $p->id }}">
                                <input type="text" value="{{ $basic->symbol }}{{ ($p->minimum + $p->maximum) / 2 }}"
                                    class="invest-type__profit--val" data-slider=".slider-input--{{ $p->id }}">
                            </div>
                            <input type="hidden" name="amount" value="{{ ($p->minimum + $p->maximum) / 2 }}"
                                class="slider-input slider-input--{{ $p->id }}" data-perday="{{ $p->percent }}"
                                data-peryear="{{ $p->time }}" data-min="{{ $p->minimum }}"
                                data-max="{{ $p->maximum }}" data-dailyprofit=".daily-profit-{{ $p->id }}"
                                data-totalprofit=".total-profit-{{ $p->id }} "
                                data-valuetag=".plan__value--{{ $p->id }} .invest-type__profit--val" />
                            <input type="hidden" name="plan_id" value="{{ $p->id }}">
                            <div class="row">
                                <div class="col-xs-6 invest-type__calc invest-type__calc--daily">
                                    <span>Per Rebeat</span>
                                    <b
                                        class="daily-profit-{{ $p->id }}">{{ $basic->symbol }}{{ ((($p->minimum + $p->maximum) / 2) * $p->percent) / 100 }}.0</b>
                                </div>
                                <div class="col-xs-6 invest-type__calc invest-type__calc--total">
                                    <span>Total Return</span>
                                    <b
                                        class="total-profit-{{ $p->id }}">{{ $basic->symbol }}{{ (((($p->minimum + $p->maximum) / 2) * $p->percent) / 100) * $p->time }}.0</b>
                                </div>
                            </div>
                ...
                @foreach ($news as $n)
                    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
                        <div class="single_news">
                            ...
                            <div class="img_holder">
                                <img class="img-responsive" src="{{ asset('assets/images') }}/{{ $n->image }}"
                                    alt="images">
                                <div class="hvr-sweep-to-bottom"></div>
                                <div class="icon_holder transition3s">
                                    <a
                                        href="{{ route('news-details', ['id' => $n->id, 'slug' => str_slug($n->title)]) }}"><i
                                            class="fa fa-link"></i></a>
                                </div>
                            </div>
                            <div class="post_meta">
                                <ul>
                                    <li><a href="#"><i class="fa fa-bars"></i> Posted On
                                            {{ $n->category->name }}</a></li>
                                    <li><a href="#"><i class="fa fa-calendar"></i>
                                            {{ \Carbon\Carbon::parse($n->created_at)->format('d F Y') }}</a></li>
                                </ul>
                                <div class="title_container">
                                    <h3>{{ $n->title }}</h3>
                                    <span class="decor_default"></span>
                                ...
                                <a href="{{ route('news-details', ['id' => $n->id, 'slug' => str_slug($n->title)]) }}"
                                    target="_blank" class="read_more transition3s">Read More <i
                        ...
                        <ul id="owl-demo2">
                            @foreach ($partner as $p)
                                <li class="item"><a href="#"><img
                                            src="{{ asset('assets/images') }}/{{ $p->image }}" alt="logo"></a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
