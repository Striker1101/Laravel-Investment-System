...
@section('content')
    <div class="col-md-12">
        <a href="{{ route('slider-create') }}" style="margin-bottom: 20px;" class="btn btn-blue pull-right">
            <i class="entypo-plus"></i> Add New Slider
        </a>
    </div>
        ...
        <tbody>
            ...
            @foreach ($slider as $p)
                ...
                <tr>
                    ...
                    <td>
                        <img width="300" src="{{ asset('assets/images') }}/{{ $p->image }}" alt="">
                    </td>
                    <td width="10%">{{ date('d-F-Y', strtotime($p->created_at)) }}</td>
                    <td width="13%">
                        <a href="{{ route('slider-edit', $p->id) }}" class="btn btn-info btn-sm">
                            <i class="fa fa-edit"></i>
                        </a>
                        <button type="button" class="btn btn-danger btn-sm delete_button" data-toggle="modal"
                            data-target="#DelModal" data-id="{{ $p->id }}">
