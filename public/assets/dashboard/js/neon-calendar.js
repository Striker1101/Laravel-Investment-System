...
(function ($, window, undefined) {
  ...
  $(document).ready(function () {
    ...
    if (neonCalendar.isPresent) {
      ...
      // Checkboxes
      var $cb = neonCalendar.$body.find(
        'table thead input[type="checkbox"], table tfoot input[type="checkbox"]'
      );
      ...
      // Highlight
      neonCalendar.$body
        .find('table tbody input[type="checkbox"]')
        .on("change", function () {
          $(this)
            .closest("tr")
            [this.checked ? "addClass" : "removeClass"]("highlight");
        });
      ...
      $("body").on("submit", "#add_event_form", function (ev) {
        ...
        if (text.val().length == 0) return false;
...
function calendar_toggle_checkbox_status(checked) {
  neonCalendar.$body
    .find('table tbody input[type="checkbox"]' + (checked ? "" : ":checked"))
